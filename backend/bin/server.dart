...
Future<void> main(List<String> args) async {
  ...
  stdout.writeln('Pica Server listening on http://${server.address.host}:${server.port}');
  stdout.writeln('Storage: ${Directory(storage).absolute.path}');
  stdout.writeln('Dotenv: ${File(dotEnvPath).existsSync() ? dotEnvPath : '(none)'}');
  stdout.writeln(apiKey == null ? 'Auth: disabled' : 'Auth: enabled (X-Api-Key)');
}
